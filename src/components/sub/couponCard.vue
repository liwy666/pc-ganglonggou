<template>
	<div class="coupon-card-main">
		<div class="coupon-box" @click="getCoupon(coupon_info.coupon_id,already_get)">
			...
			<Tooltip v-if="view_number" :content="'手快有手慢无！已领:' + already_get + '%'" style="width: 100%">
				<Progress :percent="already_get" status="active" stroke-color="#f44"/>
			</Tooltip>
		</div>
	</div>
</template>
        ...
        methods: {
            getCoupon(coupon_id, already_get) {
                ...
                if (already_get >= 100) {
                    ...
                    this.$post('user_get_coupon', {coupon_id: coupon_id, user_token: this.$store.getters.getToken})
                        .then((msg) => {
                            if (msg === "领券成功") {
