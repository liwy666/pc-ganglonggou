		...
		<div class="invoice-body">
			...
			<div v-for="(item,i) in invoice_list" :key="i" v-show="idl_switch_index.invoice_type_index === i">
				...
				<div class="form-box" v-for="(item2,i2) in item.list" :key="i2" v-show="idl_switch_index.invoice_head_index[i] === i2">
					<Input v-if="item2.form.indexOf('invoice_phone')!== -1" v-model="invoice_phone" placeholder="请输入手机号" label="手机号"/>
					<Input v-if="item2.form.indexOf('invoice_qymc')!== -1" v-model="invoice_qymc" placeholder="请输入企业名称" label="企业名称"/>
					<Input v-if="item2.form.indexOf('invoice_nsrsbh')!== -1" v-model="invoice_nsrsbh" placeholder="请输入纳税人识别号" label="税号"/>
					<Input v-if="item2.form.indexOf('invoice_kpdz')!== -1" v-model="invoice_kpdz" placeholder="请输入开票地址" label="开票地址"/>
					<Input v-if="item2.form.indexOf('invoice_zj')!== -1" v-model="invoice_zj" placeholder="请输入如:0510-80336198 座机号" label="座机号"/>
					<Input v-if="item2.form.indexOf('invoice_khh')!== -1" v-model="invoice_khh" placeholder="请输入开户行名称" label="开户行"/>
					<Input v-if="item2.form.indexOf('invoice_yhzh')!== -1" v-model="invoice_yhzh" placeholder="请输入银行账号" label="银行账号"/>
				</div>
			</div>
		</div>
	</div>
